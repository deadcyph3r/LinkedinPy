...
with smart_run(session):
    ...
    lc1, lc2 = (
        min(now.weekday() % len(location_codes), len(location_codes) - 1),
        min(len(location_codes) % (now.weekday() + 1), len(location_codes) - 1),
    )
    cc1, cc2 = (
        min(now.day % len(school_codes), len(school_codes) - 1),
        min(len(school_codes) % (now.day + 1), len(school_codes) - 1),
    )
    location_codes_today = [location_codes[lc1], location_codes[lc2]]
    school_codes_today = [school_codes[cc1], school_codes[cc2]]

    # activity
    # search and connect with filters
    ...
    session.jobs_easy_apply(position="Data Scientist", location="United States")
